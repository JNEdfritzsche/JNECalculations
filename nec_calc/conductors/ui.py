...
from nec_calc.conductors.report import render_export_report
...
def render_calc():
    ...
    if n_parallel > 1:
        m1.metric("Allowable total ampacity across N runs", f"{fmt(result.get('calculated_value'), 'A')}")
        st.caption(f"Based on {n_parallel} parallel runs × {fmt(result.get('allowable_single'), 'A')} per conductor.")
    else:
        m1.metric("Allowable ampacity per run ($I_{allowable}$)", fmt(result.get("calculated_value"), "A"))
    ...
    with d1:
        ...
        term_val = result.get("terminal_limit_ampacity")
        st.write(f"- Equipment terminal limit ($I_{{terminal}}$): **{fmt(term_val, 'A') if term_val is not None else 'None (no limit applied)'}**")
    with d2:
    ...
    if check_load or check_parallel or check_wire_type:
        ...
        with add_col2:
            if check_parallel:
                st.markdown("#### Parallel Runs ($N_{parallel}$)")
                st.number_input(
                ...
                st.write("")
                st.metric("Derated ampacity per conductor", fmt(derated_val, "A"))
