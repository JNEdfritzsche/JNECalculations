...
CF_SCHEDULE_SPEC = ReportSpec(
    ...
    cols=[
        ...
        Column("Trade size", lambda r: get_first(r, "trade_size", default="—"), group="conduit"),
        Column("Metric designator", lambda r: get_first(r, "metric_designator", default="—"), result=True, group="conduit"),
        Column("Conductors", lambda r: get_first(r, "n_conductors", default="—"), result=True),
        Column("Total conductor area", lambda r: fmt(get_first(r, "total_conductor_area"), _area_unit(r)), result=True),
        Column("Internal area (100%)", lambda r: fmt(get_first(r, "internal_area"), _area_unit(r)), result=True),
        Column("Allowed fill (Table 1)", lambda r: fmt(get_first(r, "allowed_percent"), "%"), result=True),
        Column("Allowed area", lambda r: fmt(get_first(r, "allowed_area"), _area_unit(r)), result=True),
        ...
        Column("Bend — one shot / full shoe", lambda r: _bend(r, "bend_one_shot_mm", "bend_one_shot_in"), result=True, group="bend"),
        Column("Bend — other bends", lambda r: _bend(r, "bend_other_mm", "bend_other_in"), result=True, group="bend"),
        Column("Code Edition", _edition),
        Column("Source Tables (Ch. 9)", _source_tables),
    ],
    groups={
        "conduit": Group("Trade size (metric designator)"),
        "bend": Group("Bend — one shot / other"),
    },
