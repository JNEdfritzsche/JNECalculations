...
from nec_calc.motor_protection.report import render_export_report
...
def _render_branch(c, result: dict) -> None:
    # c.markdown("##### Branch-circuit protection — NEC 430.52(C)(1)")
    ...
    c.dataframe(pd.DataFrame(rows), hide_index=True, use_container_width=True)
    c.caption(
...
def _render_overload(result: dict) -> None:
    ...
    if ol["max_overload"] is None:
        st.info("Enter the nameplate FLA (in the overload expander above) to size overload protection.")
        return
    m1, m2 = st.columns(2)
