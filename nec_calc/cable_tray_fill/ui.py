...
from nec_calc.cable_tray_fill.report import render_export_report
...
def render_calc():
    ...
    units_choice = st.radio(
        ...
        horizontal=True,
        key="ctfn_units",
        help="NEC Table 392.22 publishes both unit systems; values are read directly from the matching table columns.",
    )
    ...
    # 392.22(A)(5) and (A)(6) split on how many cables share the channel, not on their
    # size, so the size band plays no part in the check.
