...
MP_SCHEDULE_SPEC = ReportSpec(
    ...
    cols=[
        ...
        Column("Table 29 row", _row, result=True, group="row"),
        Column("Row description", lambda r: get_first(r, "row_description", default="—"), group="row"),
        Column("Device type", lambda r: get_first(r, "device_label", default="—")),
        Column("Multiplier, k", _multiplier, result=True),
        Column("OCPD raw (A)", lambda r: fmt(get_first(r, "ocpd_raw"), "A"), result=True, group="ocpd"),
        Column("Selected rating", _selected, color="green", result=True, group="ocpd"),
        Column("Code Edition", _edition),
        Column("Source Tables", _source_tables),
    ],
    groups={
        "row": Group("Table 29 row"),
        "ocpd": Group("OCPD raw → selected (A)", " → "),
    },
