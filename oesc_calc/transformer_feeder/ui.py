...
from oesc_calc.transformer_feeder.calculation import calc_transformer_feeder
from oesc_calc.transformer_feeder.report import render_add_to_schedule, render_schedule_section
...
def render_calc():
    ...
    with inputs_pane, st.container(border=True):
        st.markdown("### Inputs")
        inputs = transformer_feeder_inputs(key_prefix="oesc_transformer_feeder")
    ...
    with result_pane, st.container(border=True):
        ...
        with st.expander("Parameters & equations used", expanded=False):
            ...
            if result["phase"] == SystemTypes.THREE_PHASE.key:
                eq(r"I=\frac{VA}{\sqrt{3}\,V}")
            else:
                eq(r"I=\frac{VA}{V}")

            eq(r"\text{Turns Ratio}=\frac{V_1}{V_2}=\frac{N_1}{N_2}=\frac{I_2}{I_1}")

            st.markdown("### Feeder sizing")
            eq(r"I=1.25\cdot I_{FLC}")
            st.caption(
                "Rule 26-256 — feeder conductors supplying a transformer are sized at not "
                "less than 125% of the rated current. This calculator reports the full-load "
                "currents only."
            )
